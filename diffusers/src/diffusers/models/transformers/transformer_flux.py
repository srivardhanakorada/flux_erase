...
def _bank_add_concept_vt(
    ...
    *,
    ...
    if concept not in bank[block_index]: bank[block_index][concept] = [] #store for every concept
    _append_vt_dedup(bank[block_index][concept], vt, cos_thr=dedup_thr, max_keep=max_keep) #append if the new vector is "unique" enough (max cosine_sim with old < threshold)
...
def _vt_to_dir(vt: torch.Tensor, *, token_idx: int = 1, eps: float = 1e-8) -> torch.Tensor:
    #extracts the first token and flattens its representation     
    v = vt[0, token_idx].reshape(-1).to(torch.float32).contiguous()
    # print(v.shape) [3072]
    return v / (v.norm() + eps)  

def _orth_columns(M: torch.Tensor, *, eps: float = 1e-8) -> torch.Tensor:
    if M.numel() == 0 or M.shape[1] == 0: return M.new_zeros((M.shape[0], 0))
...
def _project_with_basis(v: torch.Tensor, B: torch.Tensor) -> torch.Tensor:
    if B is None or B.numel() == 0 or B.shape[1] == 0: return torch.zeros_like(v)
...
def _remove_subspace_component(v: torch.Tensor, U: Optional[torch.Tensor], eps: float = 1e-8) -> torch.Tensor:
    if U is None or U.numel() == 0 or U.shape[1] == 0: return v
    proj = _project_with_basis(v, U)
    out = v - proj
    if out.ndim == 1: out = out / (out.norm() + eps)
    return out

def _cora_score_and_coeff(v_free: torch.Tensor, U: torch.Tensor, eps: float) -> Tuple[torch.Tensor, torch.Tensor]:
    t = torch.einsum("dr,btd->btr", U, v_free) #projecting prompt onto erase subspace.... how much of it lies in the erase subspace
...
def _cora_erase_replace(
    ...
    *,
    ...
    if Vret is not None and Vret.numel() > 0 and Vret.shape[1] > 0:
        ...
        v_free = v32 - _FLUX_RETAIN_LAMBDA * v_pres # free the prompt from retention space....         
    else:
        v_pres = torch.zeros_like(v32)
        v_free = v32
    if U is None or U.numel() == 0 or U.shape[1] == 0: return v_slice
    t, r = _cora_score_and_coeff(v_free, U, eps=eps)           
    m = (r >= tau).to(v_free.dtype).unsqueeze(-1)  #gating... if the amount of "overlap" between prompt and the erase subspace is greater than or equal to tau, activate.              
    removed = torch.einsum("dr,btr->btd", U, t) #projection of the prompt onto erase subspace                
    v_free2 = v_free - gamma * m * removed #remove this projection scaled by gamma
    if use_replace and A is not None and A.numel() > 0 and A.shape[1] > 0:
        ...
        A_ = A_ - U_ @ (U_.t() @ A_) #is this needed?? didn't we already remove erased concept from anchor vectors while storing them??
        A_ = _orth_columns(A_, eps=eps)  
        if A_ is not None and A_.numel() > 0 and A_.shape[1] > 0:
            rA = A_.shape[1]
            rU = U_.shape[1]
            if rA == rU: added = torch.einsum("dr,btr->btd", A_, t)
            elif rA == 1:
...
def _retain_basis_from_vt_list(v_list: Optional[List[torch.Tensor]], *, top_k: int = 3) -> Optional[torch.Tensor]:
    ...
    for vt in v_list:
        d = _vt_to_dir(vt)
        M = d.unsqueeze(1) # 3072 x 1  
        u, _, _ = torch.linalg.svd(M, full_matrices=False) # SVD on a singular vector??
        # print("u shape",u.shape) 3072 x 1
        # print("M shape", M.shape) 3072 x 1
        # print("U vector", u)
        # print("M vector", M)
        imp = u[:, : min(top_k, u.shape[1])]
        U_stack = imp if U_stack is None else torch.cat([U_stack, imp], dim=1) # concatenate all first token representations of the vectors available for the given concept. 
    if U_stack is None: return None
    Vret = _orth_columns(U_stack)
    print("Vret shape: ",Vret.shape)
    return Vret
...
def flux_finalize_cora_bases(
    *,
    ...
    for blk, vlist in _FLUX_RETAIN_VT_BANK_DUAL.items():
        ...
        if Vret is not None: _FLUX_VRET_DUAL[blk] = Vret
    for blk, vlist in _FLUX_RETAIN_VT_BANK_SINGLE.items():
        Vret = _retain_basis_from_vt_list(vlist, top_k=retain_top_k)
        if Vret is not None: _FLUX_VRET_SINGLE[blk] = Vret
    def free_dir(d: torch.Tensor, Vret: Optional[torch.Tensor]) -> torch.Tensor:
        if Vret is None or Vret.numel() == 0 or Vret.shape[1] == 0: return d
        proj = Vret @ (Vret.t() @ d)
        return d - _FLUX_RETAIN_LAMBDA * proj
    for blk, concept_map in _FLUX_TARGET_VT_BANK_DUAL.items():
        Vret = _FLUX_VRET_DUAL.get(blk, None) # common retention for all erased concepts (multiconcept, dual stream)
        _FLUX_U_DUAL.setdefault(blk, {})
        for concept, vt_list in concept_map.items(): # performing this over all erased concepts (multiconcept, dual stream)
            dirs = []
            for vt in vt_list:     
                d = _vt_to_dir(vt, eps=eps)  #extracts the first token and flattens its representation, [3072]     
                d = free_dir(d, Vret)  #removes retention space component from the erase vector         
                d = d / (d.norm() + eps)
                dirs.append(d)
            if len(dirs) == 0: continue
            M = torch.stack(dirs, dim=1)         
            Uc = _orth_columns(M, eps=eps) # QR decomposition to orthogonalize all the "pure" erased concept vectors       
            _FLUX_U_DUAL[blk][concept] = Uc #orthogonalized, freed from retention space
    for blk, concept_map in _FLUX_ANCHOR_VT_BANK_DUAL.items():
        Vret = _FLUX_VRET_DUAL.get(blk, None) # common retention for all erased concepts (multiconcept, dual stream)
        _FLUX_A_DUAL.setdefault(blk, {})
        for concept, vt_list in concept_map.items(): # performing this over all anchor concepts, {}"donald trump": vectors of "man"}
            Uc = _FLUX_U_DUAL.get(blk, {}).get(concept, None) # "Pure" erased vector representations that are orthogonalized using QR decomposition
            dirs = []
            for vt in vt_list:
                a = _vt_to_dir(vt, eps=eps) #extracts the first token and flattens its representation, [3072]     
                a = free_dir(a, Vret)  #removes retention space component from the anchor vector           
                a = _remove_subspace_component(a, Uc, eps=eps) #remove any anchor component that lies along the erased subspace.
                if a.norm() <= eps: continue
                a = a / (a.norm() + eps)
                dirs.append(a)
            if len(dirs) == 0: continue
            M = torch.stack(dirs, dim=1)
            Ac = _orth_columns(M, eps=eps) # orthogonalize the columns
            if Ac is None or Ac.numel() == 0 or Ac.shape[1] == 0: continue
            _FLUX_A_DUAL[blk][concept] = Ac
    for blk, concept_map in _FLUX_TARGET_VT_BANK_SINGLE.items():
        Vret = _FLUX_VRET_SINGLE.get(blk, None)  # common retention for all erased concepts (multiconcept, single stream)
        _FLUX_U_SINGLE.setdefault(blk, {})
        for concept, vt_list in concept_map.items():# performing this over all erased concepts (multiconcept, single stream)
            dirs = []
            for vt in vt_list:
                d = _vt_to_dir(vt, eps=eps)  #extracts the first token and flattens its representation, [3072]   
                d = free_dir(d, Vret)
            ...
            _FLUX_U_SINGLE[blk][concept] = Uc #orthogonalized, freed from retention space
    for blk, concept_map in _FLUX_ANCHOR_VT_BANK_SINGLE.items():
        Vret = _FLUX_VRET_SINGLE.get(blk, None) # common retention for all erased concepts (multiconcept, single stream)
        _FLUX_A_SINGLE.setdefault(blk, {})
        for concept, vt_list in concept_map.items():# performing this over all anchor concepts (multiconcept, single stream)
            dirs = []
            for vt in vt_list:
                a = _vt_to_dir(vt, eps=eps)  #extracts the first token and flattens its representation, [3072]   
                a = free_dir(a, Vret)
    ...
    for blk, cmap in _FLUX_U_DUAL.items():
        allU = [U for U in cmap.values() if U is not None and U.numel() > 0 and U.shape[1] > 0] #accumuate freed orthogonalized erase vectors for all concepts 
        if len(allU) == 0: continue
        M = torch.cat(allU, dim=1)
        _FLUX_U_UNION_DUAL[blk] = _orth_columns(M, eps=eps) #reinforce orthogonality.... represents the final set of orthonormal vectors 
    for blk, cmap in _FLUX_U_SINGLE.items():
        allU = [U for U in cmap.values() if U is not None and U.numel() > 0 and U.shape[1] > 0] #accumuate freed orthogonalized erase vectors for all concepts 
        if len(allU) == 0: continue
        M = torch.cat(allU, dim=1)
        _FLUX_U_UNION_SINGLE[blk] = _orth_columns(M, eps=eps)#reinforce orthogonality.... represents the final set of orthonormal vectors 
    ...
    def __call__(
        ...
        **_unused,
        ...
        if encoder_hidden_states is None and text_seq_len is not None and block_index is not None:
            ...
            if single_zero_text_value: value[:, :text_seq_len] = 0.0
            if (record_retain_vt or record_target_vt or record_anchor_vt) and (block_index in target_single_block_indices):
                ...
                # print("vt_single shape: ",vt_single.shape) 1 x 512 x 24 x 128
                if record_retain_vt:
                    _bank_add_vt(
                ...
                if record_target_vt:
                    #recording plain targets (not projecting them out of retain projection space yet)....
                    if not record_concept:
                        raise ValueError("record_target_vt=True requires joint_attention_kwargs['record_concept']")
                    #perform this for every concept. 
                ...
                if record_anchor_vt:
                    ...
                    #perform this for every concept. store anchor for each target concept. currently same anchor for all, functionality can be changed
                    _bank_add_concept_vt(
        ...
        if attn.added_kv_proj_dim is not None and encoder_hidden_states is not None and block_index is not None:
            ...
            if (record_retain_vt or record_target_vt or record_anchor_vt) and (block_index in target_block_indices):
                ...
                if record_target_vt:
                    if not record_concept: raise ValueError("record_target_vt=True requires joint_attention_kwargs['record_concept']")
                    _bank_add_concept_vt(
                ...
                if record_anchor_vt:
                    if not record_concept: raise ValueError("record_anchor_vt=True requires joint_attention_kwargs['record_concept']")
                    _bank_add_concept_vt(
