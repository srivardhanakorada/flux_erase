...
def _bank_add_anchor_once_vt(
    ...
    *,
    ...
    # same structure as _bank_add_vt; kept separate for clarity
    if block_index not in bank:
        bank[block_index] = []
    _append_vt_dedup(bank[block_index], vt, cos_thr=dedup_thr, max_keep=max_keep)
...
def _cora_erase_replace(
    ...
    *,
    ...
    k_txt: Optional[torch.Tensor] = None,
    q_img: Optional[torch.Tensor] = None,
    ...
    if use_replace and A is not None and A.numel() > 0 and A.shape[1] > 0:
        ...
        # keep safety: anchor shouldn't reintroduce erased subspace
        A_ = A_ - U_ @ (U_.t() @ A_)
    ...
    return v_out.to(dtype=v_slice.dtype),v_img.to(dtype=v_slice.dtype) if v_img is not None else None, ind_ret if ind_ret is not None else None
...
def _make_attn_eos_duplicated_vt(
    ...
    *,
    ...
    A = F.softmax(logits, dim=-1)  # [1,H,L,L]
...
def flux_finalize_cora_bases(*, retain_top_k: int = 6, eps: float = 1e-8):
    global _FLUX_VRET_DUAL, _FLUX_VRET_SINGLE
    global _FLUX_U_UNION_DUAL, _FLUX_U_UNION_SINGLE
    global _FLUX_A_UNION_DUAL, _FLUX_A_UNION_SINGLE
    global _FLUX_U_DUAL, _FLUX_U_SINGLE, _FLUX_A_DUAL, _FLUX_A_SINGLE
    ...
    # keep these cleared so nothing accidentally uses them
    _FLUX_U_DUAL.clear()
    _FLUX_U_SINGLE.clear()
    _FLUX_A_DUAL.clear()
    _FLUX_A_SINGLE.clear()
...
class FluxAttnProcessor:
    ...
    def __call__(
        ...
        record_anchor_vt: bool = False,      # legacy per-target anchor recording
        record_anchor_once: bool = False,    # NEW: single anchor recording (no record_concept)
        ...
        active_concept: Optional[str] = None,  # ignored by unified mode; kept for compat
        ...
        **_unused,
        ...
        # ===========================
        # Single-stream blocks (no encoder_hidden_states)
        # ===========================
        if encoder_hidden_states is None and text_seq_len is not None and block_index is not None:
            ...
            if (record_retain_vt or record_target_vt or record_anchor_vt or record_anchor_once) and (
            ...
            if apply_target_proj and (block_index in target_single_block_indices):
                ...
                q_txt = query[:, :text_seq_len].reshape(query.shape[0], text_seq_len, -1)
                k_txt = key[:, :text_seq_len].reshape(key.shape[0], text_seq_len, -1)
                # print("v_txt shape: ",v_txt.shape)
                v_slice = v_txt[:, s:e, :]
                q_slice = q_txt[:, s:e, :]
                k_slice = k_txt[:, s:e, :]
                ...
                # print("q_txt shape: ",q_txt.shape)
                # print("k_txt shape: ",k_txt.shape)
                q_img = query[:, text_seq_len:].reshape(query.shape[0],img_tokens_len, -1).detach()[:1].contiguous()
                ...
                # print("k_img shape: ",k_img.shape)
                # print("v_img shape: ",v_img.shape)
                # unified mode: ALWAYS UNION erase + UNION anchor
                ...
                if U is not None and U.numel() > 0 and U.shape[1] > 0:
                    ...
                    if(v_img is not None):
                        value_img_new = v_img.view(value.shape[0], value.shape[1]-text_seq_len, value.shape[2], value.shape[3])
                    # print("value_img_new.shape: ",value_img_new.shape)
        ...
        if attn.added_kv_proj_dim is not None and encoder_hidden_states is not None and block_index is not None:
            ...
            if (record_retain_vt or record_target_vt or record_anchor_vt or record_anchor_once) and (
                ...
                vt_dual = _make_attn_eos_duplicated_vt(vt_dual, q_txt, k_txt, attn_mode="col",token_end=detector_token_end,)
                if record_retain_vt:
                    _bank_add_vt(
            ...
            if apply_target_proj and (block_index in target_block_indices):
                # print("Encoder Query shape:", encoder_query.shape)
                # print("Encoder Key shape:", encoder_key.shape)
                # print("Encoder Value shape:", encoder_value.shape)
                ...
                if U is not None and U.numel() > 0 and U.shape[1] > 0:
                    ...
                    A = None if A is None else A.to(device=v_slice.device, dtype=torch.float32)
                    Vret = None if Vret is None else Vret.to(device=v_slice.device, dtype=torch.float32)
                    v_txt = encoder_value.reshape(encoder_value.shape[0],encoder_value.shape[1],-1).detach()[:1].contiguous()
                    q_txt = encoder_query.reshape(encoder_query.shape[0],encoder_query.shape[1],-1).detach()[:1].contiguous()
                    k_txt = encoder_key.reshape(encoder_key.shape[0],encoder_key.shape[1],-1).detach()[:1].contiguous()
                    q_img = query.reshape(query.shape[0],query.shape[1],-1).detach()[:1].contiguous()
                    k_img = key.reshape(key.shape[0],key.shape[1],-1).detach()[:1].contiguous()
                    v_img = value.reshape(value.shape[0],value.shape[1],-1).detach()[:1].contiguous()
                    # print("q_txt shape: ",q_txt.shape)
                    # print("k_txt shape: ",k_txt.shape)
                    # print("q_img shape: ",q_img.shape)
                    # print("k_img shape: ",k_img.shape)
    ...
    def forward(
        ...
        for index_block, block in enumerate(self.single_transformer_blocks):
            ...
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                ...
                # print("hiden_states shape before single block:", hidden_states.shape)
                # print("encoder_hidden_states /shape before single block:", encoder_hidden_states.shape)
                encoder_hidden_states, hidden_states = block(
                    hidden_states=hidden_states,
                    encoder_hidden_states=encoder_hidden_states,
                    temb=temb,
