...
# analysis-only person banks / basis (not used by CORA edit rule)
...
# legacy per-target anchor banks
...
# per-concept bases (kept but cleared; not used by unified mode)
...
# ============================================================
# Analysis logging (lightweight scalar logs only)
# ============================================================

_FLUX_ANALYSIS_LOGGING_ENABLED: bool = False
_FLUX_ANALYSIS_LOGS: Dict[str, Dict[int, List[dict]]] = {
    "dual": {},
    "single": {},
}


def flux_set_analysis_logging(enabled: bool = True):
    global _FLUX_ANALYSIS_LOGGING_ENABLED
    _FLUX_ANALYSIS_LOGGING_ENABLED = bool(enabled)


def flux_reset_analysis_logs():
    global _FLUX_ANALYSIS_LOGS
    _FLUX_ANALYSIS_LOGS = {
        "dual": {},
        "single": {},
    }
...
def flux_get_analysis_logs():
    return _FLUX_ANALYSIS_LOGS


def _append_analysis_log(stream: str, block_index: int, payload: dict):
    if not _FLUX_ANALYSIS_LOGGING_ENABLED:
        return
    if block_index not in _FLUX_ANALYSIS_LOGS[stream]:
        _FLUX_ANALYSIS_LOGS[stream][block_index] = []
    _FLUX_ANALYSIS_LOGS[stream][block_index].append(payload)
...
def _summarize_cora_stats(
    *,
    ...
    if rP is None:
        return {
            "rU_mean": float(rU.mean().item()),
            "rU_max": float(rU.max().item()),
            "rP_mean": None,
            "rP_max": None,
...
def _cora_erase_replace(
    ...
    *,
    ...
    Vperson: Optional[torch.Tensor] = None,   # analysis-only, not used in edit
    tau: float,
    gamma: float,
    ...
    if U is None or U.numel() == 0 or U.shape[1] == 0:
        if return_stats:
            z = torch.zeros(v32.shape[:2], dtype=torch.float32, device=v32.device)
            g = torch.zeros((*v32.shape[:2], 1), dtype=torch.float32, device=v32.device)
            stats = _summarize_cora_stats(rU=z, gate=g, rP=z if Vperson is not None else None)
            return v_slice, stats
        return v_slice

    t, r = _cora_score_and_coeff(v_free, U, eps=eps)
    gate = (r >= tau).to(v_free.dtype).unsqueeze(-1)
    ...
    v_out = v_out.to(dtype=v_slice.dtype)

    if return_stats:
        rP = None
        if Vperson is not None and Vperson.numel() > 0 and Vperson.shape[1] > 0:
            _, rP = _cora_score_and_coeff(v_free, Vperson, eps=eps)
        stats = _summarize_cora_stats(rU=r, gate=gate, rP=rP)
        return v_out, stats
...
def flux_finalize_cora_bases(*, retain_top_k: int = 6, person_top_k: int = 6, eps: float = 1e-8):
    global _FLUX_VRET_DUAL, _FLUX_VRET_SINGLE
    global _FLUX_VPERSON_DUAL, _FLUX_VPERSON_SINGLE
    global _FLUX_U_UNION_DUAL, _FLUX_U_UNION_SINGLE
    global _FLUX_A_UNION_DUAL, _FLUX_A_UNION_SINGLE
    ...
    def build_union_U(
        ...
        for blk, concept_map in target_bank.items():
            ...
            dirs: List[torch.Tensor] = []
            for _concept, vt_list in concept_map.items():
                for vt in vt_list:
                    d = _vt_to_dir(vt, eps=eps)
                    if d is None:
                        continue
    ...
    _FLUX_U_UNION_DUAL.update(build_union_U(_FLUX_TARGET_VT_BANK_DUAL, _FLUX_VRET_DUAL))
    _FLUX_U_UNION_SINGLE.update(build_union_U(_FLUX_TARGET_VT_BANK_SINGLE, _FLUX_VRET_SINGLE))
    ...
    _FLUX_A_UNION_DUAL.update(build_union_A_once(_FLUX_ANCHOR_VT_BANK_DUAL_ONCE, _FLUX_VRET_DUAL, _FLUX_U_UNION_DUAL))
    _FLUX_A_UNION_SINGLE.update(build_union_A_once(_FLUX_ANCHOR_VT_BANK_SINGLE_ONCE, _FLUX_VRET_SINGLE, _FLUX_U_UNION_SINGLE))


# ============================================================
# Basis overlap helpers
# ============================================================

def _basis_overlap_normalized(U: Optional[torch.Tensor], V: Optional[torch.Tensor]) -> float:
    if U is None or V is None or U.numel() == 0 or V.numel() == 0:
        return 0.0
    U = U.to(torch.float32)
    V = V.to(torch.float32)
    M = V.t() @ U
    denom = max(1, min(U.shape[1], V.shape[1]))
    return float((M.pow(2).sum() / denom).item())


def flux_collect_basis_overlap():
    out = {"dual": {}, "single": {}}

    dual_blocks = sorted(set(_FLUX_U_UNION_DUAL.keys()) | set(_FLUX_VPERSON_DUAL.keys()) | set(_FLUX_VRET_DUAL.keys()))
    for blk in dual_blocks:
        out["dual"][blk] = {
            "U_vs_Vperson": _basis_overlap_normalized(
                _FLUX_U_UNION_DUAL.get(blk, None),
                _FLUX_VPERSON_DUAL.get(blk, None),
            ),
            "U_vs_Vret": _basis_overlap_normalized(
                _FLUX_U_UNION_DUAL.get(blk, None),
                _FLUX_VRET_DUAL.get(blk, None),
            ),
            "rank_U": int(_FLUX_U_UNION_DUAL[blk].shape[1]) if blk in _FLUX_U_UNION_DUAL else 0,
            "rank_Vperson": int(_FLUX_VPERSON_DUAL[blk].shape[1]) if blk in _FLUX_VPERSON_DUAL else 0,
            "rank_Vret": int(_FLUX_VRET_DUAL[blk].shape[1]) if blk in _FLUX_VRET_DUAL else 0,
        }

    single_blocks = sorted(set(_FLUX_U_UNION_SINGLE.keys()) | set(_FLUX_VPERSON_SINGLE.keys()) | set(_FLUX_VRET_SINGLE.keys()))
    for blk in single_blocks:
        out["single"][blk] = {
            "U_vs_Vperson": _basis_overlap_normalized(
                _FLUX_U_UNION_SINGLE.get(blk, None),
                _FLUX_VPERSON_SINGLE.get(blk, None),
            ),
            "U_vs_Vret": _basis_overlap_normalized(
                _FLUX_U_UNION_SINGLE.get(blk, None),
                _FLUX_VRET_SINGLE.get(blk, None),
            ),
            "rank_U": int(_FLUX_U_UNION_SINGLE[blk].shape[1]) if blk in _FLUX_U_UNION_SINGLE else 0,
            "rank_Vperson": int(_FLUX_VPERSON_SINGLE[blk].shape[1]) if blk in _FLUX_VPERSON_SINGLE else 0,
            "rank_Vret": int(_FLUX_VRET_SINGLE[blk].shape[1]) if blk in _FLUX_VRET_SINGLE else 0,
        }
...
class FluxAttnProcessor:
    ...
    def __call__(
        ...
        record_person_vt: bool = False,   # analysis-only
        ...
        **_unused,
        ...
        if encoder_hidden_states is None and text_seq_len is not None and block_index is not None:
            ...
            if (record_retain_vt or record_person_vt or record_target_vt or record_anchor_vt or record_anchor_once) and (
                ...
                vt_single = _make_attn_eos_duplicated_vt(
                    vt_single, q_txt, k_txt, attn_mode="col", token_end=detector_token_end
                )
            ...
            if apply_target_proj and (block_index in target_single_block_indices):
                ...
                Vperson = _FLUX_VPERSON_SINGLE.get(block_index, None)  # analysis only
                ...
                if U is not None and U.numel() > 0 and U.shape[1] > 0:
                    ...
                    v_slice2, stats = _cora_erase_replace(
                        v_slice,
                        Vret=Vret,
                        U=U,
                        A=A,
        ...
        if attn.added_kv_proj_dim is not None and encoder_hidden_states is not None and block_index is not None:
            ...
            if (record_retain_vt or record_person_vt or record_target_vt or record_anchor_vt or record_anchor_once) and (
                ...
                vt_dual = _make_attn_eos_duplicated_vt(
                    vt_dual, q_txt, k_txt, attn_mode="col", token_end=detector_token_end
                )
            ...
            if apply_target_proj and (block_index in target_block_indices):
                ...
                Vperson = _FLUX_VPERSON_DUAL.get(block_index, None)  # analysis only
