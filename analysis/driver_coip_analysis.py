...
from diffusers.models.transformers.transformer_flux import (  # type: ignore
    ...
    flux_finalize_coip_bases,
    flux_collect_basis_overlap,
)

MODEL_ID = "black-forest-labs/FLUX.1-schnell"
OUTDIR = "coip_analysis_outputs"
...
def main():
    ...
    # ----------------------------------------
    # 2) Record person/category banks
    # ----------------------------------------
    ...
    save_json(basis_overlap, os.path.join(OUTDIR, "basis_overlap_coip.json"))
    ...
    seed_base = 5000
    for tag, prompts in PROMPT_GROUPS.items():
        for i, prompt in enumerate(prompts):
            run_gate_probe(pipe, prompt, prompt_tag=tag, seed=seed_base + 100 * list(PROMPT_GROUPS.keys()).index(tag) + i)

    flux_set_analysis_logging(False)

    analysis_logs = flux_get_analysis_logs()
    save_json(analysis_logs, os.path.join(OUTDIR, "gate_stats_coip.json"))

    print(f"Saved basis overlap to: {os.path.join(OUTDIR, 'basis_overlap_coip.json')}")
    print(f"Saved gate stats to:   {os.path.join(OUTDIR, 'gate_stats_coip.json')}")
