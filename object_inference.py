...
PROJ_STRENGTH = 6.0
PROJ_TOKEN_END = 128
STRENGTH_TAU = 0 # check the plot of min(1,max(0,cosine_sim-tau/(1-tau))), intuitively, 0 makes the most sense since it performs edits when the cosine similarity >= 0 
STRENGTH_GAMMA = 1.0 # check th plot of (min(1,max(0,cosine_sim-tau)/(1-tau))^gamma). A larger gamma activates more for high cosine similarity and lesser for smaller cosine similarity
...
if __name__ == "__main__":
    ...
    for concept in TARGETS:
        ...
        os.makedirs(concept_dir+'/uniform/',exist_ok = True)
        os.makedirs(concept_dir+'/adaptive/',exist_ok = True)
        ...
        for i in range(N_SAMPLES):
            ...
            img_uniform.save(concept_dir+'/uniform/'+str(i)+'.png')
            kw = adaptive_kwargs_loose()
            kw.update(record_target_vt=False, apply_target_proj=True)
            img_adapt = run(pipe, prompt, seed, kw)
            img_adapt.save(concept_dir+'/adaptive/'+str(i)+'.png')
